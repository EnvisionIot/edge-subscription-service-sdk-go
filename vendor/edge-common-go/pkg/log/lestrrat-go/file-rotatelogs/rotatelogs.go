...
import (
	...
	"edge-common-go/pkg/log/lestrrat-go/strftime"
...
func New(p string, options ...Option) (*RotateLogs, error) {
	...
	for _, o := range options {
		...
		case optkeyMaxFileSize:
			maxFileSize = o.Value().(int64)
		case optkeyMaxKeepFile:
			maxFileKeepCnt = o.Value().(int)
		}
	}
	...
	r := &RotateLogs{
		...
		maxKeepFileCount: maxFileKeepCnt,
		currentFiles:     []string{},
	}
	err = r.getCurrentAllFiles()
	if err != nil {
		return nil, errors.New("options MaxAge and RotationCount cannot be both set")
	}
	r.deleteFormerFile()
...
func (rl *RotateLogs) getCurrentAllFiles() error {
	...
	for _, path := range matches {
		...
		sec := fi.ModTime().Unix()
		fileNums = append(fileNums, int(sec))
		fileMap[int(sec)] = path
	}
	sort.Ints(fileNums)
	...
	rl.currentFiles = append(rl.currentFiles, allFiles...)
	//check last file
	filename := rl.genFilename()
	lenCnt := len(rl.currentFiles)
	if lenCnt > 0 {
		lastFileName := rl.currentFiles[lenCnt-1]
		if rl.checkFileIsWriting(lastFileName, filename) {
			rl.currentFiles = rl.currentFiles[:lenCnt-1]
		}
	}
...
func (rl *RotateLogs) checkFileIsWriting(lastName, newName string) bool {
	lastList := strings.Split(lastName, optkeyContract)
	newList := strings.Split(newName, optkeyContract)
	lastCnt := len(lastList)
	newCnt := len(newList)
	if lastCnt < 2 || newCnt < 2 {
		return false
	}
	if lastList[lastCnt-1] == newList[newCnt-1] {
		return true
	}
	return false
}

func (rl *RotateLogs) deleteFormerFile() {
	fileCnt := len(rl.currentFiles)
	deleteCnt := fileCnt - rl.maxKeepFileCount
	if deleteCnt <= 0 {
		return
	}
	i := 0
	...
	for {
		...
		path := rl.currentFiles[i]
		err := os.Remove(path)
		if err != nil && !os.IsNotExist(err) {
			_, _ = fmt.Fprintf(os.Stderr, "deleteFormerFile %s failed: %s\n", path, err.Error())
		} else {
			//remove success, pop
	...
	if len(iArray) > 0 {
		rl.popStringArray(iArray)
	}
}

func (rl *RotateLogs) popStringArray(iArray []int) {
	tmpFiles := []string{}
	for key, value := range rl.currentFiles {
		if !isKeyExist(key, iArray) {
			tmpFiles = append(tmpFiles, value)
		}
	}
	rl.currentFiles = tmpFiles
}

func isKeyExist(index int, iArray []int) bool {
...
func (rl *RotateLogs) createNewLogFile() error {
	info, err := rl.outFh.Stat()
	if err != nil {
		return err
	}
	size := info.Size()
	...
	_ = rl.outFh.Close()
	sec := time.Now().Unix()
	suffixName := fmt.Sprintf("%s.%d", rl.curFn, sec)
	err = os.Rename(rl.curFn, suffixName)
	if err != nil {
		return err
	}
	rl.currentFiles = append(rl.currentFiles, suffixName)
	//new file
	filename := rl.genFilename()
...
func (rl *RotateLogs) getWriter_nolock(bailOnRotateFail, useGenerationalNames bool) (io.Writer, error) {
	...
	rl.msgCount++
	if rl.msgCount >= msgChanLimit {
		err := rl.createNewLogFile()
		if err != nil {
			return nil, err
		}
		rl.msgCount = 0
		//check if need remove old files
		rl.deleteFormerFile()
	}
	...
	if rl.curFn != filename {
		...
		if len(rl.curFn) > 0 {
			rl.msgCount = 0
			rl.currentFiles = append(rl.currentFiles, rl.curFn)
		}
	} else {
