...
func New(pt, p string, options ...Option) (*RotateLogs, error) {
	globPattern := p
	...
	datapattern, err := strftime.New(pt)
	if err != nil {
		return nil, errors.Wrap(err, `invalid strftime datapattern`)
	}
